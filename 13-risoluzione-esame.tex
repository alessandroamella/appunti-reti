\documentclass[a4paper,12pt]{article} % Tipo di documento: articolo, formato A4, font 12pt

% Pacchetti essenziali per la formattazione e il testo in italiano
\usepackage[utf8]{inputenc}  % Consente l'uso di caratteri UTF-8 (es. lettere accentate)
\usepackage[T1]{fontenc}     % Migliora la resa dei caratteri e la sillabazione
\usepackage[italian]{babel}  % Supporto per la lingua italiana (sillabazione, nomi sezioni, ecc.)

% Pacchetti per matematica
\usepackage{amsmath}         % Supporto per ambienti matematici avanzati (es. testo in formule)
\usepackage{amssymb}         % Simboli matematici aggiuntivi

% Pacchetti per liste personalizzate e indentazione
\usepackage{enumitem}        % Personalizzazione delle liste (enumerate, itemize)

% Pacchetti per layout e links
\usepackage[a4paper, margin=1in]{geometry} % Imposta i margini del documento
\usepackage{hyperref}        % Rende il sommario e i riferimenti cliccabili nel PDF
\hypersetup{
    colorlinks=true,         % Colora i link
    linkcolor=blue,          % Colore dei link interni
    urlcolor=cyan,           % Colore dei link esterni
    citecolor=green          % Colore dei riferimenti bibliografici
}

% Metadati del documento
\title{Guida alla Risoluzione degli Esercizi di Reti di Calcolatori}
\author{Il tuo Nome/Autore} % Puoi inserire il tuo nome o lasciarlo vuoto
\date{\today}                % Inserisce la data corrente, o puoi specificare una data fissa
...
\clearpage % Forza una nuova pagina prima del sommario
\tableofcontents % Genera automaticamente il sommario del documento
\clearpage % Forza una nuova pagina dopo il sommario

% Introduzione e indice delle tipologie di esercizi (come nel tuo MD, ma con riferimenti)
\section*{Introduzione alle Tipologie di Esercizi} % Sezione senza numero e non nel sommario
Questo documento è pensato per aiutarti a comprendere e risolvere le principali tipologie di esercizi pratici presenti negli esami di Reti di Calcolatori, basandosi sugli esempi forniti.

\bigskip % Piccola spaziatura verticale

\textbf{Indice delle Tipologie di Esercizi:}
\begin{enumerate}[leftmargin=*,label=\arabic*.] % Lista numerata con margine allineato
...
\begin{itemize}[leftmargin=*] % Lista puntata con margine allineato
...
\textbf{2. Subnet A (max 118 host) $\to$ /25 (128 indirizzi)}
\begin{itemize}[leftmargin=*]
...
\begin{enumerate}[leftmargin=*,label=\arabic*.]
    ...
    \begin{itemize}
        ...
        \item Meglio per M grande: Alice invia a Bob $K_{\text{pub\_B}}(K_s), K_s(M, K_{\text{priv\_A}}(H(M)))$. O più spesso: $K_{\text{pub\_B}}(K_s), K_s(M), K_{\text{priv\_A}}(H( K_s(M) ))$.
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item \texttt{CNAME (Canonical Name)}: Crea un alias per un hostname. (Es. \texttt{(cs.unibo.it, leporello.cs.unibo.it, CNAME, 10)} $\to$ \texttt{cs.unibo.it} è un alias per \texttt{leporello.cs.unibo.it})
...
\clearpage % Inizia una nuova pagina per la prossima sezione per chiarezza
...
\subsection*{Esempio Svolto (basato su Esercizio 10, "18 febbraio 2021"):}
Trovare cammino minimo A $\to$ B e tabella di routing per R4.

\textit{Il riempimento della tabella Dijkstra completa come mostrato in Markdown è un processo laborioso e va fatto con molta attenzione. La tabella in sede d'esame è spesso il risultato finale delle iterazioni. Per la visualizzazione, riporterò solo la tabella di routing finale.}

\textbf{Tabella di instradamento di R4:}
\begin{tabular}{|l|l|}
\hline
\textbf{Destinazione} & \textbf{Prossimo Hop} \\
\hline
A                     & A                     \\
R1                    & R1                    \\
R2                    & R2                    \\
